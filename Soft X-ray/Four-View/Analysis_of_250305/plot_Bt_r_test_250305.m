...
t = 451:480;
[Bt_mean,Bt_std] = deal(zeros(numel(shotList),40));
% trange = t-399;
% j = 1;
figure;hold on;
for i = 1:numel(shotList)
    shotList_tmp = cell2mat(shotList(i));
    Bt_r = zeros(numel(shotList_tmp),40);
    ...
    for k = shotList_tmp
        ...
        [Bt_r(j,:),r_vec] = get_Bt_r(grid2D,data2D,target_time);
        plot(r_vec, Bt_r(j,:), 'LineWidth', 2);
        % Br_t(j,:) = get_Br_time(grid2D,data2D,trange) .* 0.75;
        % plot(t,Br_t(j,:));
        j = j+1;
    end
    Bt_mean(i,:) = mean(Bt_r,'omitmissing');
    Bt_std(i,:) = std(Bt_r,'omitmissing');
end

figure; hold on;
for i = 1:numel(shotList)
    errorbar(r_vec,Bt_mean(i,:),Bt_std(i,:));
end
