...
% 中心位置のインデックス
centerIdx1 = [1,3,11,9];
centerIdx2 = [2,4,12,10];
centerIdx3 = [5,7,15,13];
centerIdx4 = [6,8,16,14];
...
positionIdx = reshape(1:16,4,4).';
centerIdxMatrix = rot90(reshape(1:16,4,4).',3);
...
for i=1:4
    ...
    % 切り出した画像を表示
        ...
        % cRange=[0 20];
        cRange=[0 500];
        subplot(4,4,positionIdx(centerIdxMatrix==centerIdx(1,i)));imagesc(roughImage1);title(title1);yticks([]);xticks([]);clim(cRange);
        subplot(4,4,positionIdx(centerIdxMatrix==centerIdx(2,i)));imagesc(roughImage2);title(title2);yticks([]);xticks([]);clim(cRange);
        subplot(4,4,positionIdx(centerIdxMatrix==centerIdx(3,i)));imagesc(roughImage3);title(title3);yticks([]);xticks([]);clim(cRange);
        subplot(4,4,positionIdx(centerIdxMatrix==centerIdx(4,i)));imagesc(roughImage4);title(title4);yticks([]);xticks([]);clim(cRange);
