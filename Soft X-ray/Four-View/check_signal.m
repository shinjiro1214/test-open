...
try
    websave(temp_file, url);
catch
    error('ダウンロードに失敗しました。スプレッドシートの共有設定が「リンクを知っている全員」になっているか確認してください。');
end
...
% 指定シートを読み込み
C = readmatrix(temp_file, 'Sheet', num2str(sheet_date));
...
if ismember(39,dtacq_num_list)
    filename1 = fullfile(pathname.rawdata, 'mag_probe', ['dtacq',num2str(39)], ['shot',num2str(shot(1)),'_tfshot',num2str(tfshot(1)),'.mat']);
...
if ismember(40,dtacq_num_list)
    filename2 = fullfile(pathname.rawdata, 'mag_probe', ['dtacq',num2str(40)], ['shot',num2str(shot(2)),'_tfshot',num2str(tfshot(2)),'.mat']);
...
% (Grid定義部分は描画に使わない場合は省略可能ですが、後半の補間プロット用に残すなら維持)
% ここではプロット保存が主目的なので、後半のInterpolationプロット(f5, f6)が不要なら削除推奨です。
% 今回はリクエストに従い、f1-f4の保存に注力します。

% --- 保存設定 ---
% 画面表示は全てOFF
figure_switch = ["off","off","off","off","off","off"];
r = 7;
col = 10;
y_upper_lim = 0.05;
y_lower_lim = -0.05;
...
foldername_fig = fullfile(pathname_fig, num2str(date),'doCheck');
if ~exist(foldername_fig, 'dir')
    mkdir(foldername_fig);
end

% スクリーンサイズ取得（非表示時のサイズ指定用）
scr_size = get(0, 'ScreenSize'); 

% --- Bz signal probe1-5 (f1) ---
f1 = figure('Visible', figure_switch(1), 'Position', scr_size);
for i=1:r
    for j=1:col
        idx = col*(i-1)+j;
        subplot(r,col,idx)
        if ok_bz(idx)==1
            plot(t_start:t_end,bz(t_start:t_end,idx));
        else
            plot(t_start:t_end,bz(t_start:t_end,idx),'r:')
        end   
        yline(0,'--k');
        title(num2str(2*idx-1));
        ylim([y_lower_lim y_upper_lim]);
        xticks([t_start t_end]);
    end
end
sgtitle(['Bz signal probe1-5 (Shot ' num2str(shot(1)) ')']) 
saveas(f1, fullfile(foldername_fig, ['shot', num2str(shot(1)), '_Bz1.png']));
close(f1);

% --- Bz signal probe6-10 (f2) ---
f2 = figure('Visible', figure_switch(2), 'Position', scr_size);
for i=1:r
    for j=1:col
        idx = col*(i+r-1)+j;
        subplot(r,col,(i-1)*col+j) % subplotのインデックスは1から埋める
        if ok_bz(idx)==1
            plot(t_start:t_end,bz(t_start:t_end,idx))
        else
            plot(t_start:t_end,bz(t_start:t_end,idx),'r:')
...
sgtitle(['Bz signal probe6-10 (Shot ' num2str(shot(1)) ')'])
saveas(f2, fullfile(foldername_fig, ['shot', num2str(shot(1)), '_Bz2.png']));
close(f2);

% --- Bt signal probe1-5 (f3) ---
f3 = figure('Visible', figure_switch(3), 'Position', scr_size);
for i=1:r
    for j=1:col
        idx = col*(i-1)+j;
        subplot(r,col,idx)
        if ok_bt(idx)==1
            plot(t_start:t_end,bt(t_start:t_end,idx))
        else
            plot(t_start:t_end,bt(t_start:t_end,idx),'r:')
        end   
        yline(0,'--k');
        title(num2str(2*idx));
        ylim([-0.2 0.2]);
...
sgtitle(['Bt signal probe1-5 (Shot ' num2str(shot(1)) ')'])
saveas(f3, fullfile(foldername_fig, ['shot', num2str(shot(1)), '_Bt1.png']));
close(f3);

% --- Bt signal probe6-10 (f4) ---
f4 = figure('Visible', figure_switch(4), 'Position', scr_size);
for i=1:r
    for j=1:col
        idx = col*(i+r-1)+j;
...
sgtitle(['Bt signal probe6-10 (Shot ' num2str(shot(1)) ')'])
saveas(f4, fullfile(foldername_fig, ['shot', num2str(shot(1)), '_Bt2.png']));
close(f4);

% f5, f6 (Interpolation plots) は switchがoffのため処理されません。
% 必要であれば同様のsaveas処理を追加してください。
