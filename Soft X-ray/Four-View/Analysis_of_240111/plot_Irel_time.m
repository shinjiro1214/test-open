...
sxrMaxList_err(1,5) = 2*sxrMaxList_err(1,5);
sxrMaxList_err(3,[4,5]) = 5*sxrMaxList_err(3,[4,5]);
...
for time = thomsonTimeList
    ...
    % r_idx_list = find(abs(r(1,:) - xPointList.r(timeIndex)) <= 0.03);
    r_idx_list = find(abs(r(1,:) - xPointList.r(timeIndex)) <= 0.04);
    % その点の平均値を取得
    ...
    ne_x = mean([ne_mat(z_idx_list,r_idx_list)],'all','omitnan')-2e19;
...
% errorbar(timeList(4:6),sxrMaxList(1,4:6)./max(sxrMaxList(1,4:6)),sxrMaxList_err(1,4:6),'LineWidth',3);
errorbar(timeList(4:6),sxrMaxList(3,4:6)./max(sxrMaxList(3,4:6)),sxrMaxList_err(3,4:6),'Color',"#EDB120",'LineWidth',3);
