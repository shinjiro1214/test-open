...
shotList = 7:30;
Br_r = zeros(numel(shotList),40);
j=1;
figure;hold on;
for i = shotList
    load([dirPath,num2str(i,'%03i'),'.mat'],'data2D','grid2D');
    [Br_profile,r_vec] = get_Br_r(grid2D,data2D,target_time);
    plot(r_vec, Br_profile, 'LineWidth', 2);
    Br_r(j,:) = Br_profile.';
    j=j+1;
end
...
% 3. 統計計算とエラーバープロット
% =========================================================================
figure('Name', 'Br Radial Profile by Group', 'Color', 'w');
hold on;
...
for k = 1:length(groups)
    ...
    groupData = Br_r(currentIdx, :);
    
    % --- 統計量の計算 ---
    N = size(groupData, 1);            % データ数 (ショット数)
    mu = mean(groupData, 1);           % 平均値
    sigma = std(groupData, 0, 1);      % 標準偏差
...
ylabel('B_r [T]', 'FontSize', 14);
title(['B_r Radial Distribution (Mean \pm SE) at t = ' num2str(target_time) ' \mu s'], 'FontSize', 16);
grid on;
...
function [Br_profile,r_vec] = get_Br_r(grid2D,data2D,target_time)
