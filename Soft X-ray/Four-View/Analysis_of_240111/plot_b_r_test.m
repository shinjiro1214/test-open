%% 設定
target_time = 465; % プロットしたい時間 [us]
% target_time = 472; % プロットしたい時間 [us]
...
shotList = 7:30;
% shotList = [20 21 30];
b_r = zeros(numel(shotList),40);
j=1;
figure;hold on;
for i = shotList
    load([dirPath,num2str(i,'%03i'),'.mat'],'data2D','grid2D');
    [b_profile,r_vec] = get_b_r(grid2D,data2D,target_time);
    plot(r_vec, b_profile, 'LineWidth', 2);
    b_r(j,:) = b_profile.';
    j=j+1;
end
...
% 3. 統計計算とエラーバープロット
% =========================================================================
figure('Name', 'b Radial Profile by Group', 'Color', 'w');
hold on;
...
title(['b Radial Distribution (Mean \pm SE) at t = ' num2str(target_time) ' \mu s'], 'FontSize', 16);
grid on;
...
function [b_profile,r_vec] = get_b_r(grid2D,data2D,target_time)
    ...
    if ndims(data2D.Bz) == 3
        ...
        if isfield(data2D,'Bt_th')
            ...
            % Bt_slice_2D = data2D.Bt(:, :, t_idx);
            Bt_slice_2D = nan(size(Bz_slice_2D));
        end
        b_slice_2D = Bt_slice_2D./sqrt(Bz_slice_2D.^2+Br_slice_2D.^2);
        % Zを固定してR方向のデータを抽出 (すべての行, 指定のZ列)
        b_profile = b_slice_2D(:, z_idx);
    else
