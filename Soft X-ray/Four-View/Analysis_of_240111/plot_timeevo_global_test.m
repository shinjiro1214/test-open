...
matrixPathArray = strings(1,4);
matrixPathArray(1) = [pathFirstHalf,'29/2.mat'];
matrixPathArray(2) = [pathFirstHalf,'10/2.mat'];
matrixPathArray(3) = [pathFirstHalf,'27/3.mat'];
matrixPathArray(4) = [pathFirstHalf,'10/3.mat'];
...
for i = 1:4
    ...
    % 負の要素を0で置換
    negativeEE = find(EE1<0);
    EE1(negativeEE) = zeros(size(negativeEE));
    EE_q = griddata(z_space_SXR2,r_space_SXR,EE1(:,:),psi_mesh_z,psi_mesh_r);
    cRange = cell2mat(cLimList(1));
    [~,h] = contourf(psi_mesh_z-0.03,psi_mesh_r,EE_q,linspace(cRange(1),cRange(2),20));clim(cRange);
    colormap('turbo');
    h.LineStyle = 'none';
