...
output_dir = "data_spheromak_gradshafranov"
...
# ==============================================================================
# --- 3. 初期条件：Grad-Shafranov Solverで平衡解を計算 ---
# ==============================================================================
print("Solving Grad-Shafranov equation for initial equilibrium...")

# --- ソルバーのパラメータ ---
GS_MAX_ITER = 2000      # 最大反復回数
GS_TOLERANCE = 1e-7     # 収束判定の許容誤差
GS_RELAX_FACTOR = 0.8   # 緩和係数（収束を安定させる）
lambda_val = 150.0      # J_y = lambda * Ay の係数。この値で平衡の形が変わる。

# --- A_y の初期推測 ---
# 以前の方法で使った形状を初期値にすると収束が速い
x_cnt = 0.0
z_cnt = LZ / 2.0
ra = 0.2 * LZ
r_sq = (x_grid - x_cnt)**2 + (z_grid - z_cnt)**2
Ay = np.exp(-r_sq / (ra**2))
Ay[0, :] = Ay[-1, :] = Ay[:, 0] = Ay[:, -1] = 0.0 # 境界条件を適用

# --- 反復計算ループ ---
for it in range(GS_MAX_ITER):
    Ay_old = Ay.copy()
    
    # 1. 現在のAyからJ_yを計算 (J_y = lambda * Ay)
    J_y = lambda_val * Ay
    
    # 2. J_yをソース項としてポアソン方程式を解き、新しいAyを得る
    Ay_new = solve_poisson_dirichlet(-J_y, dx, dz) # ソース項の符号に注意
    
    # 3. Ayの規格化 (固有値問題では振幅が発散するため、毎回規格化する)
    ay_max = np.max(np.abs(Ay_new))
    if ay_max > 1e-9:
        Ay_new /= ay_max
        
    # 4. 緩和法を用いて解を更新
    Ay = GS_RELAX_FACTOR * Ay_new + (1.0 - GS_RELAX_FACTOR) * Ay_old
    
    # 5. 収束判定
    # 差分のL2ノルムを計算
    diff = np.sqrt(np.sum((Ay - Ay_old)**2)) / np.sqrt(np.sum(Ay**2))
    if it % 50 == 0:
        print(f"  GS Iter: {it:4d},  Relative Difference: {diff:.4e}")
    
    if diff < GS_TOLERANCE:
        print(f"✅ Grad-Shafranov solver converged after {it+1} iterations.")
        break
else:
    print(f"⚠️ Grad-Shafranov solver did not converge within {GS_MAX_ITER} iterations.")

# --- 求解したAyを用いて磁場を計算 ---
# この部分は元のコードと同じ
print("Setting up fields from the GS solution...")
# Ayが求まったので、これからポロイダル磁場を計算
Bz_temp, Bx_temp = np.gradient(Ay, dx, dz, edge_order=2)
B[:, :, 0] = Bx_temp
B[:, :, 2] = -Bz_temp

# トロイダル磁場もAyに比例させる (F(psi)がpsiに比例するモデル)
By_peak = -5.0 * np.max(np.abs(B[:,:,2]))
ay_max = np.max(np.abs(Ay))
if ay_max > 1e-9:
    # 2つに分離させる場合は、Ayを2つの山に分離する操作が必要になるが、
    # まずは単一のスフェロマックで実装する
    B[:, :, 1] = By_peak * (Ay / ay_max)
else:
    B[:, :, 1] = 0.0

# マスクは不要になるか、あるいは求解したAyの形状に基づいて作成する
spheromak_mask = Ay > 0.1 * np.max(Ay) # 例：Ayがピーク値の10%以上の領域をマスク
B[:, :, 1] = np.where(spheromak_mask, B[:, :, 1], 0.0)

print("Initial fields have been set from Grad-Shafranov solution.")
# ==============================================================================
# --- 粒子初期化 ---
# ==============================================================================
...
def push_and_deposit_species(pos_s, vel_s, charge_s, mass_s, E, B, J_s, rho_s, wp, dt, Lx, Lz, dx, dz, NX, NZ, hit_count_array):
    J_s.fill(0); rho_s.fill(0)
    inv_vol = 1.0/(dx*dz)
    for i in range(pos_s.shape[0]):
        ix_f=(pos_s[i,0]+Lx/2)/dx; iz_f=pos_s[i,1]/dz
        if not (0<=ix_f<NX-1 and 0<=iz_f<NZ-1): hit_count_array[0]+=1; continue
        ...
        pos_s[i,0]+=vel_s[i,0]*dt; pos_s[i,1]+=vel_s[i,2]*dt
        pos_s[i,0]=np.mod(pos_s[i,0]+Lx/2,Lx)-Lx/2
        if pos_s[i,1]>Lz: pos_s[i,1]=2*Lz-pos_s[i,1]; vel_s[i,2]*=-1.0
        if pos_s[i,1]<0: pos_s[i,1]=-pos_s[i,1]; vel_s[i,2]*=-1.0
        ix_f_new=(pos_s[i,0]+Lx/2)/dx; iz_f_new=pos_s[i,1]/dz
        if not (0<=ix_f_new<NX-1 and 0<=iz_f_new<NZ-1): hit_count_array[0]+=1; continue
        ix,iz=int(ix_f_new),int(iz_f_new); wx,wz=ix_f_new-ix,iz_f_new-iz
        dwx,dwz=1-wx,1-wz; q_contrib=charge_s[i]*wp*inv_vol
        rho_s[ix,iz]+=q_contrib*dwx*dwz; rho_s[ix+1,iz]+=q_contrib*wx*dwz
        rho_s[ix,iz+1]+=q_contrib*dwx*wz; rho_s[ix+1,iz+1]+=q_contrib*wx*wz
        for d in range(3):
            j_p=q_contrib*vel_s[i,d]
            J_s[ix,iz,d]+=j_p*dwx*dwz; J_s[ix+1,iz,d]+=j_p*wx*dwz
            J_s[ix,iz+1,d]+=j_p*dwx*wz; J_s[ix+1,iz+1,d]+=j_p*wx*wz
...
if N_total > 0:
    ...
    _, _, _, rho_init = push_and_deposit_species(pos, vel_thermal, charge, mass, np.zeros_like(E), B, J_plasma, rho, wp, 0, LX, LZ, dx, dz, NX, NZ, continue_hit_counter_array)
