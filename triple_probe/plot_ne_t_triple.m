...
ne_data_all = squeeze(triple_data2D.ne(:, 1, :)); % [eV] (保存時に変換済みと仮定)
...
% --- 4. プロット ---
figure('Name', 'Spatially Averaged ne Evolution', 'Color', 'w');

% エラーバー付きでプロット (空間的なばらつきを表示)
% 塗りつぶし(shaded error bar)で見やすくする場合
hold on;
curve1 = ne_spatial_mean + ne_spatial_std;
curve2 = ne_spatial_mean - ne_spatial_std;
...
% 平均値を太線でプロット
plot(time_values, ne_spatial_mean, 'r', 'LineWidth', 2, 'DisplayName', 'Spatial Mean');
...
%     filename = sprintf('ne_avg_time_evolution_%d', target_date);
