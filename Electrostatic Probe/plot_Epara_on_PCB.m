...
% dPhi = squeeze( max(E_data.phi_perp,[],[1 2]) - min(E_data.phi_perp,[],[1 2]) );
% figure;plot(t_plot,dPhi,'k','LineWidth',2);xlim([460 475]);
...
Epara = cat(3,Epara_r,Epara_z,Epara_t);
plotRange = [-700 700];
titleList = {'E_r x B_r','E_z x B_z','E_t x B_t'};
figure;
for m = 1:3
    % i=start+m.*dt; %end
    i = t_plot_idx;
...
function [E_data,B_data] = get_Epara(grid2D,data2D,ESP,ESPdata2D,t_plot)
    [Epara,E_r,E_z,E_t,B_r,B_z,B_t,phi_perp,Epara_perp] = deal(zeros([size(ESPdata2D.phi_mesh_r),numel(t_plot)]));
    ...
    E_data.Epara = Epara;E_data.Epara_perp = Epara_perp;
